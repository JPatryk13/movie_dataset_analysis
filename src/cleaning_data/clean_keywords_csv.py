...
# keywords_df -> done, no duplicates, correct types, no nulls
keywords_df = all_keywords_df[['keyword_id', 'name']].drop_duplicates(ignore_index=True).sort_values('keyword_id')

# keywords_movies_junction_df -> done, correct types, no nulls
all_keywords_df = all_keywords_df[['film_id', 'keyword_id']].sort_values('film_id').astype({'film_id': 'int64'})
